...
| **Hyvä** | Supported. Every storefront placement works, including the nested display and the variant switch on the product page — the module ships its own script for those, no setup needed. Checked on a Hyvä storefront (Magento 2.4.8) with a simple and a configurable product. Two points are up to your agency: the GARAN label on the product page has to be anchored in the theme (the pattern is in the module's `README.md`, section *Themes*), and the checkout runs through Hyvä's Luma fallback checkout, so it behaves as it does in Luma. **Hyvä Checkout is not supported**: the notice and the GARAN list do not appear there, because it does not load the module's Knockout components. |
