...
| **Hyvä** | Unterstützt. Alle Platzierungen in der Storefront funktionieren, auch die geschachtelte Anzeige und der Variantenwechsel auf der Produktseite – dafür liefert das Modul ein eigenes Skript mit, ohne Einrichtungsaufwand. Geprüft auf einer Hyvä-Storefront (Magento 2.4.8) mit einem einfachen und einem konfigurierbaren Produkt. Zwei Punkte übernimmt Ihre Agentur: Das GARAN-Label auf der Produktseite muss im Theme verankert werden (Muster in der `README.md` des Moduls, Abschnitt *Themes*), und der Checkout läuft über Hyväs Luma-Fallback-Checkout und verhält sich daher wie in Luma. **Hyvä Checkout wird nicht unterstützt**: Hinweis und GARAN-Liste erscheinen dort nicht, weil die Knockout-Komponenten des Moduls nicht geladen werden. |
